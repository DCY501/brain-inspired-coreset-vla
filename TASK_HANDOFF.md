...
| 排名 | 方法 | 思路 | MSE | MAE | 状态 |
|------|------|------|-----|-----|------|
| 🥇 | **密度过滤 FPS (fr=0.15)** | 思路四 | **0.003792** | 0.0339 | ✅ 全局最优 |
...
| 7 | **互补融合 v3.0** | 融合 v3.0 | **0.004429** | 0.0365 | ❌ 语义盲区补充无效 |
| 8 | **分层融合 cr2.0_dfps** | 融合 v2.0 | **0.005272** | 0.0376 | ❌ 密度过滤 FPS 在子空间内失效 |
| 9-12 | 得分融合 v1.0 (4组) | 融合 v1.0 | 0.0054~0.0095 | — | ❌ 均失败 |
| — | 时序事件边界 | 思路二 | 0.008871 | 0.0612 | ❌ 单任务时序耦合 |
| — | AE 重建误差 | 思路三 | ~0.007~0.008 | — | ❌ CLIP 特征太规整 |

**随机基线**：5 次运行 avg MSE=0.007229 ± 0.000862

### 密度过滤 FPS filter_ratio 消融实验

| filter_ratio | 替换帧数 | Test MSE | 结论 |
|-------------|---------|---------|------|
| 0.05 | 80 | 0.004196 | 修正不足 |
| 0.10 | 160 | 0.004725 | 仍不足 |
| **0.15** | **240** | **0.003792** | **🥇 最优 sweet spot** |
| 0.20 | 320 | 0.004899 | 过度修正 |
| 0.25 | 400 | 0.004840 | 严重过度 |

**关键发现**：0.15 是最优 sweet spot——替换太少不足以修正 FPS 稀疏偏差，替换太多破坏覆盖优势。
...
### 🟡 中优先级：密度过滤 FPS filter_ratio 消融实验 —— ✅ 已完成

**问题**：验证密度过滤 FPS 中 `filter_ratio=0.15` 是否是最优 sweet spot。

**方法**：固定 k=5，测试 filter_ratio ∈ {0.05, 0.10, 0.15, 0.20, 0.25}。

**结果**：
- 0.05: MSE=0.004196（修正不足）
- 0.10: MSE=0.004725（仍不足）
- **0.15: MSE=0.003792（🥇 最优）**
- 0.20: MSE=0.004899（过度修正）
- 0.25: MSE=0.004840（严重过度）

**结论**：0.15 是最优 sweet spot。替换太少不足以修正 FPS 稀疏偏差，替换太多破坏覆盖优势。详见 `approaches/farthest_point_sampling/README.md` v4.3 节。

### 🟢 低优先级：代码推送

- `git add` 所有新文件（approaches/ 下的代码、结果）
- `git commit` 提交当前状态
- `git push` 到 GitHub（上次 push 因 HTTPS 认证失败，需手动处理）
...
6. **互补融合 v3.0 语义盲区补充无效**：以密度过滤 FPS 为基础集，v1.1 发现 138 个语义盲区簇，替换 138 帧后 MSE 反而上升到 0.004429。**关键洞察**：语义稀有 ≠ 动作预测价值高，密度过滤 FPS 的"冗余"定义（局部密度高）比 v1.1 的"语义稀有"更贴近动作预测需求。单一最优方法的信息完整性已经足够，额外的语义补充反而引入噪声、破坏几何覆盖。
7. **分层融合上限**：分层融合 v2.0 的最佳表现（cr2.0: 0.004004）介于 v1.1（0.003835）和标准 FPS（0.004294）之间，但无法超越单一最优方法。说明两层结构的"信息漏斗"效应：v1.1 粗筛不可避免地过滤掉了部分有用帧，FPS 只能在剩余候选池内优化，无法恢复被过滤的信息。
...
4. ✅ **更新 README.md** —— 已完成：更新目录结构、补充实验结果表格、添加 cr2.0_dfps 和互补融合分析
5. ⏳ **git commit + push** —— 待完成：提交所有修改到 GitHub（上次 push 因 HTTPS 认证失败）
