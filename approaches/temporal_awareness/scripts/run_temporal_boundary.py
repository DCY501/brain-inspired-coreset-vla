"""
时序事件边界采样独立实验脚本
利用动作变化率检测事件边界 -> 训练 MLP -> 测试集评估 MSE
"""
import os
import sys
...
from approaches.temporal_awareness.src.selector_temporal_boundary import TemporalEventBoundarySelector
from train_mlp import MLPRegressor, train_model
from evaluate import evaluate_model


def run_single_temporal(seed: int = RANDOM_SEED):
    """运行一次时序事件边界采样实验"""
    np.random.seed(seed)
    ...
    print("\n" + "=" * 60)
    print(f"Temporal Event Boundary Sampling | Seed = {seed}")
    print("=" * 60)
    ...
    print(f"[Data] Episodes: {len(np.unique(ep_train))} train | "
          f"{len(np.unique(ep_indices[test_mask]))} test")
    
    selector = TemporalEventBoundarySelector(target_ratio=CORESET_RATIO)
    coreset_idx = selector.select(None, y_train_full, ep_train)
    ...
    save_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', f"temporal_seed{seed}.pt"
    )
    ...
    print("\n[Results] Temporal Test MSE: {:.6f} | MAE: {:.6f}".format(results['mse'], results['mae']))
    
    result_dict = {
        'method': 'temporal_event_boundary',
        'seed': seed,
    ...
    out_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', f"temporal_result_seed{seed}.json"
    )
...
def main():
    res = run_single_temporal(seed=RANDOM_SEED)
    print("\n" + "=" * 60)
    print(f"Temporal Event Boundary Test MSE: {res['test_mse']:.6f}")
    print("=" * 60)
