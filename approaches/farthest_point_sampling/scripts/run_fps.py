...
def run_single_fps(seed: int = RANDOM_SEED):
    """运行一次最远点采样实验"""
    np.random.seed(seed)
    ...
    print("\n" + "=" * 60)
    print(f"Farthest Point Sampling | Seed = {seed}")
    print("=" * 60)
    ...
    selector = FarthestPointSamplingSelector(target_ratio=CORESET_RATIO, seed=seed)
    ...
    save_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', f"fps_seed{seed}.pt"
    )
    ...
    print("\n[Results] FPS Test MSE: {:.6f} | MAE: {:.6f}".format(results['mse'], results['mae']))
    ...
    out_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', f"fps_result_seed{seed}.json"
    )
...
def main():
    res = run_single_fps(seed=RANDOM_SEED)
    print("\n" + "=" * 60)
    print(f"FPS Test MSE: {res['test_mse']:.6f}")
    print("=" * 60)
