"""
密度过滤 FPS 独立实验脚本
Density-Filtered FPS -> 训练 MLP -> 测试集评估 MSE
"""
import os
import sys
...
def run_single(seed: int = RANDOM_SEED):
    ...
    print("\n" + "=" * 60)
    print(f"Density-Filtered FPS | Seed = {seed}")
    print("=" * 60)
    ...
    selector = DensityFilteredFPSSelector(target_ratio=CORESET_RATIO, seed=seed)
    ...
    save_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', f"density_filtered_fps_seed{seed}.pt"
    )
    ...
    print("\n[Results] Density-Filtered FPS Test MSE: {:.6f} | MAE: {:.6f}".format(
    ...
    out_path = os.path.join(
        ...
        f"density_filtered_fps_result_seed{seed}.json"
    )
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"Density-Filtered FPS Test MSE: {res['test_mse']:.6f}")
    print("=" * 60)
