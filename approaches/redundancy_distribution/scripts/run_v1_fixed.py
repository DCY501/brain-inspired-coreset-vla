...
from approaches.redundancy_distribution.src.selector_v1_fixed import BrainInspiredCoresetSelector
...
def run_single_coreset(seed: int = RANDOM_SEED):
    """运行一次 v1.0 核心集实验"""
    print("\n" + "=" * 60)
    print(f"Coreset v1.0 (Fixed Quota) | Seed = {seed}")
    print("=" * 60)
    ...
    save_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', 'v1_fixed', f"coreset_v1_seed{seed}.pt"
    )
    ...
    print("\n[Results] Coreset v1.0 Test MSE: {:.6f} | MAE: {:.6f}".format(results['mse'], results['mae']))
    
    result_dict = {
        'method': 'brain_inspired_coreset_v1.0_fixed',
        'seed': seed,
    ...
    out_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', 'v1_fixed', f"coreset_v1_result_seed{seed}.json"
    )
...
def main():
    ...
    print("\n" + "=" * 60)
    print(f"Coreset v1.0 Test MSE: {res['test_mse']:.6f}")
    print("=" * 60)
