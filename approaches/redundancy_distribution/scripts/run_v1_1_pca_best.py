"""
v1.1 + PCA 400d 最优配置实验脚本
PCA 400d 降维 -> K-Means 动态保底 -> 训练 MLP -> 测试集评估 MSE
"""
import os
import sys
...
def run_single_coreset(seed: int = RANDOM_SEED, pca_components: int = 400):
    """运行一次 v1.1 + PCA 核心集实验"""
    np.random.seed(seed)
    ...
    print("\n" + "=" * 60)
    print(f"Coreset v1.1 + PCA{pca_components} | Seed = {seed}")
    print("=" * 60)
    ...
    selector = BrainInspiredCoresetSelector(target_ratio=CORESET_RATIO, pca_components=pca_components)
    ...
    save_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', 'pca_sweep',
        f"coreset_pca{pca_components}_seed{seed}.pt"
    )
    ...
    print("\n[Results] Coreset v1.1+PCA{} Test MSE: {:.6f} | MAE: {:.6f}".format(
        pca_components, results['mse'], results['mae']))
    
    result_dict = {
        'method': f'brain_inspired_coreset_v1.1_pca{pca_components}',
        'seed': seed,
    ...
    out_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', 'pca_sweep',
        f"coreset_pca{pca_components}_result_seed{seed}.json"
    )
...
def main():
    res = run_single_coreset(seed=RANDOM_SEED, pca_components=400)
    print("\n" + "=" * 60)
    print(f"Coreset v1.1 + PCA400 Test MSE: {res['test_mse']:.6f}")
    print("=" * 60)
