"""
预测编码滑动窗口独立实验脚本
Predictive Coding Sliding Window -> 训练 MLP -> 测试集评估 MSE
"""
import os
import sys
...
from approaches.predictive_coding_sliding_window.src.selector_predictive_coding import PredictiveCodingSelector
from train_mlp import MLPRegressor, train_model
from evaluate import evaluate_model


def run_single(seed: int = RANDOM_SEED):
    """运行一次预测编码滑动窗口实验"""
    np.random.seed(seed)
    ...
    print("\n" + "=" * 60)
    print(f"Predictive Coding Sliding Window | Seed = {seed}")
    print("=" * 60)
    ...
    # 预测编码选择器不需要视觉特征，只需要动作和 episode 索引
    # features 参数传空（或不传），保留接口一致性
    visual_train = X_train_full[:, :VISUAL_FEATURE_DIM]
    
    selector = PredictiveCodingSelector(
        target_ratio=CORESET_RATIO, window=5, seed=seed, fusion_mode='residual_delta'
    )
    ...
    save_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', f"predictive_coding_seed{seed}.pt"
    )
    ...
    print("\n[Results] Predictive Coding Test MSE: {:.6f} | MAE: {:.6f}".format(
        results['mse'], results['mae']))
    
    result_dict = {
        'method': 'predictive_coding_sliding_window',
        'seed': seed,
    ...
    out_path = os.path.join(
        ...
        f"predictive_coding_result_seed{seed}.json"
    )
...
def main():
    res = run_single(seed=RANDOM_SEED)
    print("\n" + "=" * 60)
    print(f"Predictive Coding Test MSE: {res['test_mse']:.6f}")
    print("=" * 60)
