...
from approaches.fusion_score_weighted.src.selector_complementary_fusion import ComplementaryFusionSelector
from train_mlp import MLPRegressor, train_model
from evaluate import evaluate_model


def run_single(coverage_threshold: float, seed: int = RANDOM_SEED):
    """运行指定覆盖率阈值的互补融合实验"""
    np.random.seed(seed)
    ...
    print("\n" + "=" * 70)
    print(f"Complementary Fusion | coverage_threshold={coverage_threshold} | Seed={seed}")
    print("=" * 70)
    ...
    # 加载预计算的密度过滤 FPS 基础集（避免重新计算，节省时间）
    fps_result_path = os.path.join(
        os.path.dirname(__file__), '..', '..', 'farthest_point_sampling', 'results',
        'density_filtered_fps_result_seed42.json'
    )
    if os.path.exists(fps_result_path):
        with open(fps_result_path, 'r') as f:
            fps_result = json.load(f)
        base_selected = fps_result.get('coreset_indices', None)
        print(f"[Preload] Loaded Density-Filtered FPS coreset: {len(base_selected)} frames")
    else:
        base_selected = None
        print("[Preload] No pre-computed FPS result found, will compute on-the-fly")
    
    selector = ComplementaryFusionSelector(
    ...
    coreset_idx = selector.select(visual_train, y_train_full, ep_train, base_selected=base_selected)
    ...
    tag = f"complementary_ct{coverage_threshold}"
    ...
    print(f"\n[Results] coverage_threshold={coverage_threshold} Test MSE: {results['mse']:.6f} | MAE: {results['mae']:.6f}")
...
def main():
    ...
    for th in thresholds:
        res = run_single(coverage_threshold=th, seed=RANDOM_SEED)
        all_results.append(res)
    
    print("\n" + "=" * 70)
    print("Complementary Fusion Summary")
    print("=" * 70)
    for r in all_results:
        print(f"  coverage_threshold={r['coverage_threshold']:.2f}: Test MSE = {r['test_mse']:.6f}")
