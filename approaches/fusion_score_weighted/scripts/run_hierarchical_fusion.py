...
from approaches.fusion_score_weighted.src.selector_hierarchical_fusion import HierarchicalFusionSelector
from train_mlp import MLPRegressor, train_model
from evaluate import evaluate_model


def run_single(candidate_ratio: float, seed: int = RANDOM_SEED):
    """运行指定候选池比例的融合实验"""
    np.random.seed(seed)
    ...
    print("\n" + "=" * 70)
    print(f"Hierarchical Fusion v2.0 | candidate_ratio={candidate_ratio} | Seed={seed}")
    print("=" * 70)
    ...
    selector = HierarchicalFusionSelector(
    ...
    tag = f"hierarchical_cr{candidate_ratio}"
    ...
    print(f"\n[Results] candidate_ratio={candidate_ratio} Test MSE: {results['mse']:.6f} | MAE: {results['mae']:.6f}")
...
def main():
    ...
    all_results = []
    for ratio in ratios:
        res = run_single(candidate_ratio=ratio, seed=RANDOM_SEED)
        all_results.append(res)
    
    print("\n" + "=" * 70)
    print("Hierarchical Fusion v2.0 Summary")
    print("=" * 70)
    for r in all_results:
        print(f"  candidate_ratio={r['candidate_ratio']:.1f}: Test MSE = {r['test_mse']:.6f}")
