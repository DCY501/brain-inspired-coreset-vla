"""
自监督重建误差采样独立实验脚本
训练 AE -> 计算重建误差 -> 选 Top-K -> 训练 MLP -> 测试集评估 MSE
"""
import os
import sys
...
def run_single_ae(seed: int = RANDOM_SEED):
    """运行一次自监督重建误差采样实验"""
    np.random.seed(seed)
    ...
    print("\n" + "=" * 60)
    print(f"Autoencoder Reconstruction Error Sampling | Seed = {seed}")
    print("=" * 60)
    ...
    selector = AutoencoderReconstructionSelector(target_ratio=CORESET_RATIO)
    ...
    save_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', f"ae_recon_seed{seed}.pt"
    )
    ...
    print("\n[Results] AE Reconstruction Test MSE: {:.6f} | MAE: {:.6f}".format(
    ...
    out_path = os.path.join(
        os.path.dirname(__file__), '..', 'results', f"ae_recon_result_seed{seed}.json"
    )
...
def main():
    res = run_single_ae(seed=RANDOM_SEED)
    print("\n" + "=" * 60)
    print(f"AE Reconstruction Test MSE: {res['test_mse']:.6f}")
    print("=" * 60)
